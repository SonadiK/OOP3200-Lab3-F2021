...
//ReadString function definition
std::string ConsoleInput::ReadString()
{
	...
	{
		std::cout << "Invalid input. Please enter at least one character.\n";
		std::cin.clear();
		std::cin.sync();
		std::cin.ignore();
	}
	return stringInput;
	// calling the function to get user input if the input is invalid.
